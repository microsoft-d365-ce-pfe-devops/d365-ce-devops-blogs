...
```
#### Download Solution Packager
In the following sample code we begin our actual build script by creating a tools directory, downlaoding the latest version of Nuget, and installing Dynamics  365 CE Core Tools. To keep things clean and in logical order after we install nuget we will move the tools we need to the tools\CoreTools diretory, this step will become reusable in the future so lets avoid the need to refactor. 
```YAML
steps:
- script: md tools
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTE2MzM4MDUsNTg3NDc1ODcxLDIwMj
